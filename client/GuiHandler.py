...
##Fönster för att skriva in ip och port
...
##hämtar port och ip från entry sätter in dom i self.ipandporttoreturn och sen retunerar dom och stänger fönstert
...
##Startar hela gui
    def startMainGui(self):
        ...
        self.root.mainloop()
##Skickar medelandet du skrivit in
    def sendMsgBySocketHandler(self):
        self.socketHandler.sendMsg(self.entryOfUser.get())
##Startar login fönsteret om du vill logga in eller registrera
...
##Funktion för att logga in tar username och password
...
#här skickas det till servern och stänger fönsteret
        def confirmLogin():
        ...
        but.grid(row = 2, column = 0)
#bara gui fönster med 4 entrys för registrering
...
#Skickar entrys till servern för registrering
        def confirmRegister():
            username = entryOfUsername.get()
            password = entryOfPassword.get()
    ...
    def startGui(self):
        ...
        if self.chattIsAllowed == True:
            self.startMainGui()
#Function som gör att du får chatta om du loggat in samt ger dig medelande om det var något fel med logginen/registreringen
